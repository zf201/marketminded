...
test('ResearchAgent returns error if llmCall returns null (no submit tool)', function () {
    ...
    $agent = new class extends ResearchAgent {
        protected function llmCall(string $sp, array $t, string $m, float $temp, bool $ust, ?string $key, int $to = 120, string $baseUrl = 'https://openrouter.ai/api/v1', string $provider = 'openrouter', ?\App\Services\BraveSearchClient $braveSearchClient = null, ?callable $onToolCall = null, string $reasoningEffort = 'medium'): ?array
        {
            return null;
        }
    };
